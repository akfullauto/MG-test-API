...
function getpdf(param, req, res) {
   ...
   req.on('end', function () {
      ...
      try { fs.writeFileSync(dir + cfg, D.Data.replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&amp;/g, "&"), 'utf8'); } // Data contains XML entities for < > & by default, except set ExportPDF_Xml='1'
...
function set(table, param, req, res) {
   try {
      ...
      if (param.event == 0) {
         ...
         let sql = `DELETE FROM Headers WHERE Id = ?`;
         const stmt = db.prepare(sql);
         stmt.run(param.id);
      } else if (param.event == 4) {
         const jsonString = JSON.stringify(param.json)
         console.log(param.json);
         const fs = require('fs');
         fs.writeFile('./DataTable.json', param.json, err => { 
             if (err) { 
...
function get(table, param, req, res) {
   ...
   try {
      var sql = require('better-sqlite3')
      let db = new sql('./SQLite.db'); // It uses SQLite3 database engine on database in file SQLite.sb
      if (param.table == 'TableData') {
         let sql = `SELECT * FROM Headers`;
         const heads = db.prepare(sql).all();
         //fullRes.push(heads);

         const fs = require('fs');
         let rawdata = fs.readFileSync('DataTable.json');
         let dataTable = JSON.parse(rawdata);
         console.log(dataTable);
         //fullRes.push(dataTable);
         const fullRes = {
            heads : heads,
            dataTable : dataTable
         }
